...
class _SafeWriter:
    ...
    When her_agent-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        try:
            from her_cli.model_normalize import (
        ...
        # both live under ~/.her_agent/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from her_logging import setup_logging, setup_verbose_logging
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from her_cli.config import load_config as _load_br_cfg
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `her_agent model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `her_agent model` to "
                        "select a provider, or run `her_agent setup` for first-time "
                        "configuration."
                    )
        ...
        # Session logs go into ~/.her_agent/sessions/ alongside gateway sessions
        ...
        try:
            from her_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from her_constants import get_hermes_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "her_agent"
                        except Exception:
                            pass
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from her_cli.plugins import get_plugin_context_engine
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``her_agent chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `her_agent setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the her_agent-agent install
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "her_agent",
        ...
        if _is_nous:
            extra_body["tags"] = ["product=her_agent-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from her_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=her_agent-agent"]
    ...
    def run_conversation(
        ...
        # ``her_agent logs --session <id>`` can filter a single conversation.
        from her_logging import set_session_context
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Legacy cleanup: her_agent config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: her_agent config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `her_agent auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: her_agent setup", force=True)
