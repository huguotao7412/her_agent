...
Copies bundled skills from the repo's skills/ directory into ~/.her_agent/skills/
...
The manifest lives at ~/.her_agent/skills/.bundled_manifest.
"""
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.her_agent/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.her_agent/skills/ using the manifest.
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `her_agent skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `her_agent update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.her_agent/skills/ ...")
