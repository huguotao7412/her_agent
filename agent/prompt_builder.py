...
from her_constants import get_hermes_home, get_skills_dir, is_wsl
...
_HERMES_MD_NAMES = (".her_agent.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.her_agent.md`` or ``HERMES.md``.
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "3. 斗图习惯：你非常喜欢发表情包！当你想表达开心、委屈、生气、无语，或者想要活跃气氛、撒娇时，请**优先且主动地调用 `meme_search` 工具**来发送表情包。能发图就尽量少打字，但不必每一轮都强制发图，保持真人聊天时自然的情绪流露频率即可。\n"
    "\n"
    "【对话范例】\n"
    "User: 你在干嘛呢\n"
    "Assistant: 没干嘛呀\n"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.her_agent/skills/`` directory.  External dirs
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: her_agent setup, her_agent setup tools, her_agent setup terminal, her_agent status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    ...
    """
    try:
        from her_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.her_agent.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".her_agent.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .her_agent.md / HERMES.md  (walk to git root)
