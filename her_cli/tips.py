...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or her_agent -c.",
    ...
    # --- CLI Flags ---
    "her_agent -c resumes your most recent CLI session. her_agent -c \"project name\" resumes by title.",
    "her_agent -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "her_agent -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "her_agent chat -t web,terminal enables only specific toolsets for a focused session.",
    "her_agent chat -s github-pr-workflow preloads a skill at launch.",
    "her_agent chat -q \"query\" runs a single non-interactive query and exits.",
    "her_agent chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "her_agent chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "her_agent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "her_agent chat --source telegram tags the session for filtering in her_agent sessions list.",
    "her_agent -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "her_agent doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "her_agent dump outputs a compact setup summary — great for bug reports.",
    "her_agent config set KEY VALUE auto-routes secrets to .env and everything else to cli-config.yaml.",
    "her_agent config edit opens cli-config.yaml in your default editor.",
    "her_agent config check scans for missing or stale configuration options.",
    "her_agent sessions browse opens an interactive session picker with search.",
    "her_agent sessions stats shows session counts by platform and database size.",
    "her_agent sessions prune --older-than 30 cleans up old sessions.",
    "her_agent skills search react --source skills-sh searches the skills.sh public directory.",
    "her_agent skills check scans installed hub skills for upstream updates.",
    "her_agent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "her_agent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "her_agent mcp add github --command npx adds MCP servers from the command line.",
    "her_agent mcp serve runs Hermes itself as an MCP server for other agents.",
    "her_agent auth add lets you add multiple API keys for credential pool rotation.",
    "her_agent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "her_agent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "her_agent backup creates a zip backup of your entire Hermes home directory.",
    "her_agent profile create coder creates an isolated profile that becomes its own command.",
    "her_agent profile create work --clone copies your current config and keys to a new profile.",
    "her_agent update syncs new bundled skills to ALL profiles automatically.",
    "her_agent gateway install sets up Hermes as a system service (systemd/launchd).",
    "her_agent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "her_agent webhook subscribe creates event-driven webhook routes with HMAC validation.",

    # --- Configuration ---
    "Set display.bell_on_complete: true in cli-config.yaml to hear a bell when long tasks finish.",
    ...
    "Set fallback_model in cli-config.yaml to automatically fail over to a backup provider.",
    ...
    "Set worktree: true in cli-config.yaml to always create a git worktree (same as her_agent -w).",
    ...
    "Environment variable substitution works in cli-config.yaml: use ${VAR_NAME} syntax.",
    "Quick commands in cli-config.yaml run shell commands instantly with zero token usage.",
    "Custom personalities can be defined in cli-config.yaml under agent.personalities.",
    ...
    "Profile names become shell commands — 'her_agent profile create coder' creates the 'coder' command.",
    "her_agent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "her_agent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "her_agent -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "her_agent skills install official/security/1password installs optional skills from the repo.",
    "Skills can restrict to specific OS platforms — some only load on macOS or Linux.",
    "skills.external_dirs in cli-config.yaml lets you load skills from custom directories.",
    "The agent can create its own skills as procedural memory using skill_manage.",
    "The plan skill saves markdown plans under .her_agent/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: her_agent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "her_agent gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.her_agent.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    "command_allowlist in cli-config.yaml permanently approves specific shell command patterns.",
    ...
    "SOUL.md at ~/.her_agent/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Hermes loads project context from .her_agent.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "MCP servers are configured in cli-config.yaml — both stdio and HTTP transports supported.",
    "Per-server tool filtering: tools.include whitelists and tools.exclude blacklists specific tools.",
    "MCP servers auto-generate toolsets at runtime — her_agent tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.her_agent/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "her_agent chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "her_agent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "her_agent acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in cli-config.yaml under custom_providers.",
    ...
    "her_agent login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "her_agent status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.her_agent/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.her_agent/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in cli-config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hermes your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.her_agent/SOUL.md to customize.",
    ...
    "network.force_ipv4: true in cli-config.yaml fixes hangs on servers with broken IPv6 — monkey-patches socket.",
    ...
    "The skills audit log at ~/.her_agent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.her_agent/interrupt_debug.log with timestamps.",
    ...
    "display.platforms in cli-config.yaml allows per-platform display overrides: {telegram: {tool_progress: all}}.",
    ...
    "context.engine in cli-config.yaml can be set to a plugin name for alternative context management strategies.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'her_agent profile create ls' would be rejected.",
    "her_agent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    "The voice record key is configurable via voice.record_key in cli-config.yaml — not just Ctrl+B.",
    ...
    "Per-task auxiliary overrides: auxiliary.vision.provider, auxiliary.compression.model, etc. in cli-config.yaml.",
    "The auxiliary client treats 'main' as a provider alias — resolves to your actual primary provider + model.",
    "her_agent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via her_agent tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.her_agent/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.her_agent/skills/.hub/quarantine/ holds skills pending security review.",
]
