...
Profiles live under ``~/.her_agent/profiles/<name>/`` by default.

The "default" profile is ``~/.her_agent`` itself — backward compatible,
zero migration needed.

Usage::

    her_agent profile create coder          # fresh profile + bundled skills
    her_agent profile create coder --clone  # also copy config, .env, SOUL.md
    her_agent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    her_agent -p coder chat                 # or via flag
    her_agent profile use coder             # set as sticky default
    her_agent profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See her_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.her_agent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "her_agent-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "her_agent", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the her_agent root, NOT to the current HERMES_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HERMES_HOME points outside
    ``~/.her_agent``, profiles live under ``HERMES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    ...
    In standard deployments this is ``~/.her_agent``.
    In Docker/custom deployments where HERMES_HOME is outside ``~/.her_agent``
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.her_agent
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, her_agent subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a her_agent subcommand"
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.her_agent)."
        )
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.her_agent).\n"
            "To remove everything, use: her_agent uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from her_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.her_agent/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: her_agent profile create {name}"
        )
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.her_agent``.
    Returns the profile name if HERMES_HOME points into ``~/.her_agent/profiles/<name>``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.her_agent itself — its parent is ~/ and its
        # directory name is ".her_agent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: her_agent profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.her_agent itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.her_agent). "
            "Specify a different name: her_agent profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for her_agent profile names."""
    return '''# Hermes Agent profile completion
# Add to ~/.bashrc: eval "$(her_agent completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.her_agent/profiles"
...
complete -F _hermes_completion her_agent
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for her_agent profile names."""
    return '''#compdef her_agent
# Hermes Agent profile completion
# Add to ~/.zshrc: eval "$(her_agent completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.her_agent/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.her_agent/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any her_agent modules
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: her_agent profile create {profile_name}"
        )
