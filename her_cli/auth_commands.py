...
import her_cli.auth as auth_mod
...
def _get_custom_provider_names() -> list:
    ...
    try:
        from her_cli.config import get_compatible_custom_providers, load_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `her_agent auth
    ...
    raise SystemExit(f"`her_agent auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from her_cli.config import remove_env_value
    ...
    # re-seeded on the next load_pool() call.
        ...
        print("Run `her_agent auth add openai-codex` to re-enable if needed.")
        ...
        from her_cli.auth import suppress_credential_source
        ...
        print("Run `her_agent auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `her_agent auth` is called bare."""
