...
- ``her_cli.providers``        -- canonical provider identity + overlays
- ``her_cli.model_normalize``  -- per-provider name formatting
...
# The previous substring check (`"her_agent" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``her_agent-brain:qwen3-14b-ctx16k`` that just
# happen to carry "her_agent" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, her_agent-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   her_agent-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'her_agent model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'her_agent doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from her_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in her_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from her_cli/models.py (OPENROUTER_MODELS,
    ...
    from her_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by her_agent provider ID
    ...
    from her_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                from her_cli.auth import _load_auth_store
    ...
    # with `her_agent model`).
    try:
        from her_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see her_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, her_cli/config.py) already consume that dict.
