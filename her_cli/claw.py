"""her_agent claw — OpenClaw migration commands.

Usage:
    her_agent claw migrate              # Preview then migrate (always shows preview first)
    her_agent claw migrate --dry-run    # Preview only, no changes
    her_agent claw migrate --yes        # Skip confirmation prompt
    her_agent claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    her_agent claw cleanup              # Archive leftover OpenClaw directories
    her_agent claw cleanup --dry-run    # Preview what would be archived
"""
...
from her_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'her_agent stop'.")
    print()
...
def claw_command(args):
    """Route her_agent claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: her_agent claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'her_agent claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: her_agent claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: her_agent claw migrate --yes")
            return
    ...
    # 'her_agent claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: her_agent claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: her_agent claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  her_agent claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  her_agent claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  her_agent config set OPENROUTER_API_KEY sk-or-v1-...")
