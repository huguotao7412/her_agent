...
Config files are stored in ~/.her_agent/ for easy access.
"""
...
from her_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from her_constants import get_optional_skills_dir
...
# display_hermes_home imported lazily at call sites (stale-module safety during her_agent update)
...
from her_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  her_agent config set model.provider custom")
    print_info("  her_agent config set model.base_url http://localhost:8080/v1")
    print_info("  her_agent config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'her_agent setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from her_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'her_agent setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'her_agent setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'her_agent setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'her_agent setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'her_agent setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('her_agent setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('her_agent setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('her_agent setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('her_agent setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('her_agent setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('her_agent config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('her_agent config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('her_agent config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('her_agent', Colors.GREEN)}              Start chatting")
    print(f"   {color('her_agent gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('her_agent doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `her_agent tools` and `her_agent setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``her_agent model``)
    ...
    provider added to ``her_agent model`` is automatically available here.
    ...
    # Delegate to the shared her_agent model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from her_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: her_agent model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from her_cli.auth_commands import auth_add_command
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'her_agent setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.her_agent/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"her_agent setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'her_agent setup tts')."""
    _setup_tts_provider(config)
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `her_agent setup agent` later to customize.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'her_agent whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'her_agent whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from her_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from her_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from her_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from her_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from her_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from her_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from her_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from her_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    from her_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  her_agent config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'her_agent setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     her_agent config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  her_agent gateway")
        else:
            from her_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   her_agent gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   her_agent gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `her_agent setup tools` and `her_agent tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from her_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from her_cli.auth import get_active_provider
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: her_agent claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: her_agent claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use her_agent claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      her_agent setup           — full or quick (auto-detected)
      her_agent setup model     — just model/provider
      her_agent setup tts       — just text-to-speech
      her_agent setup terminal  — just terminal backend
      her_agent setup gateway   — just messaging platforms
      her_agent setup tools     — just tool configuration
      her_agent setup agent     — just agent settings
    """
    ...
    if is_existing:
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'her_agent setup' again when ready.")
            return
        elif 2 <= choice <= 6:
    ...
    print()
    print_info("You can edit these files directly or use 'her_agent config edit'")
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``her_agent chat`` in a fresh process."""
    hermes_bin = shutil.which("her_agent")
    if hermes_bin:
        return [hermes_bin, "chat"]

    try:
        if importlib.util.find_spec("her_cli") is not None:
            return [sys.executable, "-m", "her_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch her_agent chat now?", True):
        return

    chat_argv = _resolve_hermes_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Hermes automatically. Run 'her_agent chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'her_agent setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    her_agent setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  her_agent setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'her_agent setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'her_agent setup gateway'.")
