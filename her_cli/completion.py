"""Shell completion script generation for her_agent CLI.
...
# Add to ~/.bashrc:
#   eval "$(her_agent completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef her_agent
# Hermes Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(her_agent completion zsh)"
...
{top_cmds_str}
            ...
            _describe 'her_agent command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "# Add to your config:",
        "#   her_agent completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.her_agent/profiles",
        "        ls $HOME/.her_agent/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c her_agent -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c her_agent -f -s p -l profile"
