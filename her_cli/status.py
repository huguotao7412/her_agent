"""
Status command for her_agent CLI.
...
from her_cli.auth import AuthError, resolve_provider
from her_cli.colors import Colors, color
from her_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from her_cli.models import provider_label
from her_cli.nous_subscription import get_nous_subscription_features
from her_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    try:
        from her_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: her_agent model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: her_agent model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: her_agent model)"
    ...
    if not terminal_env:
        ...
        # (her_agent status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    try:
        from her_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   her_agent gateway")
    ...
    print(color("  Run 'her_agent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'her_agent setup' to configure", Colors.DIM))
    print()
