...
Usage:
    her_agent                     # Interactive chat (default)
    her_agent chat                # Interactive chat
    her_agent gateway             # Run gateway in foreground
    her_agent gateway start       # Start gateway as service
    her_agent gateway stop        # Stop gateway service
    her_agent gateway status      # Show gateway status
    her_agent gateway install     # Install gateway service
    her_agent gateway uninstall   # Uninstall gateway service
    her_agent setup               # Interactive setup wizard
    her_agent logout              # Clear stored authentication
    her_agent status              # Show status of all components
    her_agent cron                # Manage cron jobs
    her_agent cron list           # List cron jobs
    her_agent cron status         # Check if cron scheduler is running
    her_agent doctor              # Check configuration and dependencies
    her_agent honcho setup                    # Configure Honcho AI memory integration
    her_agent honcho status                   # Show Honcho config and connection status
    her_agent honcho sessions                 # List directory → session name mappings
    her_agent honcho map <name>               # Map current directory to a session name
    her_agent honcho peer                     # Show peer names and dialectic settings
    her_agent honcho peer --user NAME         # Set user peer name
    her_agent honcho peer --ai NAME           # Set AI peer name
    her_agent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    her_agent honcho mode                     # Show current memory mode
    her_agent honcho mode [hybrid|honcho|local]  # Set memory mode
    her_agent honcho tokens                   # Show token budget settings
    her_agent honcho tokens --context N       # Set session.context() token cap
    her_agent honcho tokens --dialectic N     # Set dialectic result char cap
    her_agent honcho identity                 # Show AI peer identity representation
    her_agent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    her_agent honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    her_agent version             Show version
    her_agent update              Update to latest version
    her_agent uninstall           Uninstall Hermes Agent
    her_agent acp                 Run as an ACP server for editor integration
    her_agent sessions browse     Interactive session picker with search

    her_agent claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (her_agent tools, her_agent setup, her_agent model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'her_agent {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any her_agent module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.her_agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check active_profile in the her_agent root
    if profile_name is None:
        try:
            from her_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from her_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent her_agent from starting
            print(
...
# Load .env from ~/.her_agent/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from her_cli.config import get_hermes_home
from her_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Initialize centralized file logging early — all `her_agent` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from her_logging import setup_logging as _setup_logging
...
try:
    from her_cli.config import load_config as _load_config_early
    from her_constants import apply_ipv4_preference as _apply_ipv4
...
from her_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from her_cli.config import get_env_path, get_hermes_home, load_config
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'her_agent')
    """
    import shutil
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo her_agent {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo her_agent {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str]) -> None:
    ...
    print(f"  her_agent --tui --resume {target}")
    if title:
        print(f'  her_agent --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @her_agent/ink is missing or node_modules is behind package-lock.json (post-pull)."""
    ink = root / "node_modules" / "@her_agent" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _hermes_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "her_agent-ink"
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".her_agent")
    try:
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        if _hermes_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/her_agent-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
                ...
                print("@her_agent/ink build failed.")
                if preview:
                    print(preview)
                sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'her_agent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("  Run:  her_agent setup")
        print()

        from her_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'her_agent setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from her_cli.banner import prefetch_update_check
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from her_cli.config import get_env_value, save_env_value
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: her_agent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  her_agent gateway")
        ...
        print()
        print("  Or install as a service: her_agent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'her_agent whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``her_agent model``) and the setup wizard
    ...
    from her_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.her_agent/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.her_agent/.env if the active provider is not 'custom'.
    ...
    """
    from her_cli.config import get_env_value, save_env_value, load_config
...
# `her_agent model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `her_agent model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from her_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``her_agent model`` flow, then route aux tasks to them here.
    """
    from her_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from her_cli.auth import _prompt_model_selection
    from her_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from her_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from her_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from her_cli.nous_subscription import prompt_enable_tool_gateway
    import argparse
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from her_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from her_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `her_agent auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from her_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from her_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.her_agent/cli-config.yaml.
    """
    from her_cli.auth import (
    ...
    from her_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from her_cli.auth import _save_model_choice, deactivate_provider
    from her_cli.config import get_env_value, load_config, save_config
    ...
    from her_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `her_agent model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from her_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from her_cli.config import save_env_value, load_config, save_config
    from her_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from her_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from her_cli.copilot_auth import validate_copilot_token
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so her_agent knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "ollama-cloud":
        from her_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from her_constants import display_hermes_home as _dhh_fn
    ...
    try:
        ...
        print("    4. Re-run:               her_agent model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from her_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'her_agent model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_logout(args):
    ...
    from her_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from her_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from her_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from her_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from her_cli.webhook import webhook_command
...
def cmd_doctor(args):
    ...
    from her_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from her_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from her_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from her_cli.config import config_command
...
def cmd_version(args):
    ...
    try:
        from her_cli.banner import check_for_updates
        from her_cli.config import recommended_update_command
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``her_agent update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``her_agent web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (her_agent web will not be available)")
        )
        if fatal:
...
def _update_via_zip(args):
    ...
    try:
        tmp_dir = tempfile.mkdtemp(prefix="her_agent-update-")
        zip_path = os.path.join(tmp_dir, f"her_agent-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to her_agent-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"her_agent-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "her_agent-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes her_agent completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `her_agent update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/hermes-agent.git",
    "git@github.com:NousResearch/her_agent-agent.git",
    "https://github.com/NousResearch/hermes-agent",
    "git@github.com:NousResearch/her_agent-agent",
}
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/her_agent-agent.")
        print()
        try:
            response = (
...
def _invalidate_update_cache():
    ...
    ``her_agent update``, every profile is now current.
    """
    homes = []
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "her_agent-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``her_agent update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.her_agent/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``her_agent update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``her_agent update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.her_agent/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``her_agent update --gateway``) the update is already
...
def cmd_update(args):
    ...
    """
    from her_cli.config import is_managed, managed_error
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_hermes_home added to her_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload her_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import her_constants as _hc
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'her_agent config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: her_agent config migrate")
            else:
                print()
                print("Skipped. Run 'her_agent config migrate' later to configure.")
        else:
        ...
        # When running as ``her_agent update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart her_agent-gateway`` kills everything in the
        ...
        try:
            ...
            # Discover all her_agent-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. her_agent-gateway.service or her_agent-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: her_agent gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: her_agent -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both her_agent.service (from a pre-rename install) and the
        # current her_agent-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `her_agent update` surfaces the issue until the user migrates.
        try:
            from her_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("  These pre-rename units (her_agent.service) fight the current")
                print("  her_agent-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    her_agent gateway migrate-legacy")
                print()
        ...
        print("  her_agent model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``her_agent -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from her_constants import display_hermes_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `her_agent profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → her_agent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.her_agent)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  her_agent profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     her_agent -p {name} chat")
                else:
        ...
        from her_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec her_agent -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from her_cli.profiles import rename_profile
        ...
        from her_cli.profiles import export_profile
        ...
        from her_cli.profiles import import_profile
...
def cmd_completion(args, parser=None):
    ...
    from her_cli.completion import generate_bash, generate_zsh, generate_fish
...
def main():
    """Main entry point for her_agent CLI."""
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    her_agent                        Start interactive chat
    her_agent chat -q "Hello"        Single query mode
    her_agent -c                     Resume the most recent session
    her_agent -c "my project"        Resume a session by name (latest in lineage)
    her_agent --resume <session_id>  Resume a specific session by ID
    her_agent setup                  Run setup wizard
    her_agent logout                 Clear stored authentication
    her_agent auth add <provider>    Add a pooled credential
    her_agent auth list              List pooled credentials
    her_agent auth remove <p> <t>    Remove pooled credential by index, id, or label
    her_agent auth reset <provider>  Clear exhaustion status for a provider
    her_agent model                  Select default model
    her_agent config                 View configuration
    her_agent config edit            Edit config in $EDITOR
    her_agent config set model gpt-4 Set a config value
    her_agent gateway                Run messaging gateway
    her_agent -s her_agent-agent-dev,github-auth
    her_agent -w                     Start in isolated git worktree
    her_agent gateway install        Install gateway background service
    her_agent sessions list          List past sessions
    her_agent sessions browse        Interactive session picker
    her_agent sessions rename ID T   Rename/title a session
    her_agent logs                   View agent.log (last 50 lines)
    her_agent logs -f                Follow agent.log in real time
    her_agent logs errors            View errors.log
    her_agent logs --since 1h        Lines from the last hour
    her_agent debug share             Upload debug report for support
    her_agent update                 Update to latest version

For more help on a command:
    her_agent <command> --help
""",
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: her_agent-cli)",
    )
    model_parser.add_argument(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy her_agent.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Hermes gateway unit files "
            "(e.g. her_agent.service) left over from older installs. Profile "
            "units (her_agent-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        "Run a specific section: her_agent setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: her_agent-cli)"
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hermes Agent. Use 'her_agent debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    her_agent debug share              Upload debug report and print URL
    her_agent debug share --lines 500  Include more log lines
    her_agent debug share --expire 30  Keep paste for 30 days
    her_agent debug share --local      Print report locally (no upload)
    her_agent debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'her_agent debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        ...
        "skills, sessions, and data (excludes the her_agent-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/her_agent-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    def cmd_pairing(args):
        from her_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.her_agent/skills/.bundled_manifest) "
            "so future 'her_agent update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from her_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from her_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/her_agent-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `her_agent plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from her_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from her_constants import get_hermes_home, display_hermes_home
            ...
            from her_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'her_agent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # her_agent tools list [--platform cli]
    ...
    # her_agent tools disable <name...> [--platform cli]
    ...
    # her_agent tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from her_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from her_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'her_agent mcp add' to connect to a new server, or\n"
            "'her_agent mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    def cmd_mcp(args):
        from her_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch her_agent --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil

            hermes_bin = shutil.which("her_agent")
            if hermes_bin:
                os.execvp(hermes_bin, ["her_agent", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "her_cli.main", "--resume", selected_id],
                )
    ...
    def cmd_claw(args):
        from her_cli.claw import claw_command
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    her_agent logs                    Show last 50 lines of agent.log
    her_agent logs -f                 Follow agent.log in real time
    her_agent logs errors             Show last 50 lines of errors.log
    her_agent logs gateway -n 100     Show last 100 lines of gateway.log
    her_agent logs --level WARNING    Only show WARNING and above
    her_agent logs --session abc123   Filter by session ID
    her_agent logs --component tools  Only show tool-related lines
    her_agent logs --since 1h         Lines from the last hour
    her_agent logs --since 30m -f     Follow, starting from 30 min ago
    her_agent logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``her_agent -c Pokemon Agent Dev`` → ``her_agent -c 'Pokemon Agent Dev'``
    ...
    from her_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'her_agent -c model' where 'model' is consumed as the
