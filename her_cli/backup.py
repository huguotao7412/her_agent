"""
Backup and import commands for her_agent CLI.

`her_agent backup` creates a zip archive of the entire ~/.her_agent/ directory
(excluding the her_agent-agent repo and transient files).

`her_agent import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
from her_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "her_agent-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to her_agent root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"her_agent-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"her_agent-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside her_agent root
            try:
    ...
    print(f"\nRestore with: her_agent import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a her_agent home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.her_agent/cli-config.yaml` instead of `cli-config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a her_agent dir name
        if prefix in (".her_agent", "her_agent"):
            return prefix + "/"
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # her_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: her_agent profile list  (after installing her_agent)")

        # Guidance
        print()
        if not (hermes_root / "her_agent-agent").is_dir():
            print("Note: The her_agent-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: her_agent update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  her_agent -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and her_agent backup --quick)
# ---------------------------------------------------------------------------
...
def run_quick_backup(args) -> None:
    """CLI entry point for her_agent backup --quick."""
