...
- Keep data: Remove code but keep ~/.her_agent/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing her_agent-agent or her_agent PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hermes Agent" comment and following line
                if '# Hermes Agent' in line or '# her_agent-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('her_agent' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing her_agent
                if 'her_agent' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the her_agent wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "her_agent",
        Path("/usr/local/bin/her_agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'her_cli' in content or 'her_agent-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``her_agent gateway run`` processes
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``her_agent -p <name> gateway stop|uninstall`` because
    ...
    #    Use `python -m her_cli.main` so we don't depend on a `her_agent`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "her_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.her_agent/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.her_agent/ for future reinstall
    """
    ...
    print(f"  Config:  {hermes_home / 'cli-config.yaml'}")
    ...
    log_info("Removing her_agent command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.her_agent/, just remove the her_agent-agent subdir
    ...
    # 5. Optionally remove ~/.her_agent/ data directory (and named profiles)
    if full_uninstall:
