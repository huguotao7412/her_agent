...
- ~/.her_agent/cli-config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.her_agent/.env         - API keys and secrets

This module provides:
- her_agent config          - Show current configuration
- her_agent config edit     - Open config in editor
- her_agent config set      - Set a specific value
- her_agent config wizard   - Re-run setup wizard
"""
...
from her_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade her_agent-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "her_agent update"
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            "Edit services.her_agent-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_name = info.get("container_name", "her_agent-agent")
    exec_user = info.get("exec_user", "her_agent")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/her_agent")
...
# Re-export from her_constants — canonical definition lives there.
from her_constants import get_hermes_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the her_agent group can
...
def ensure_hermes_home():
    """Ensure ~/.her_agent directory structure exists with secure permissions.
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #  "/home/user/.her_agent/cache/documents:/output"]
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.her_agent/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.her_agent/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for her_agent-specific overrides; everything else
    ...
    "code_execution": {
        ...
        #     her_agent-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.her_agent/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @her_agent:example.org)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'her_agent-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.her_agent.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load cli-config.yaml", "Run 'her_agent setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'her_agent doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.her_agent")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            try:
                from her_constants import get_hermes_home as _ghome
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `her_agent plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: her_agent config set <key> <value>")
    ...
    # metadata.her_agent.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.her_agent/cli-config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.her_agent/cli-config.yaml."""
    ensure_hermes_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — her_agent auth) — OpenAI Codex
#   nous         (OAuth — her_agent auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.her_agent/cli-config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.her_agent/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.her_agent/.env in place.
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.her_agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.her_agent/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.her_agent/.env or environment."""
...
def show_config():
    ...
    print(color("  her_agent config edit     # Edit config file", Colors.DIM))
    print(color("  her_agent config set <key> <value>", Colors.DIM))
    print(color("  her_agent setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: her_agent config set <key> <value>")
            print()
            print("Examples:")
            print("  her_agent config set model anthropic/claude-sonnet-4")
            print("  her_agent config set terminal.backend docker")
            print("  her_agent config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'her_agent config migrate' to add them")
        ...
        print("  her_agent config           Show current configuration")
        print("  her_agent config edit      Open config in editor")
        print("  her_agent config set <key> <value>   Set a config value")
        print("  her_agent config check     Check for missing/outdated config")
        print("  her_agent config migrate   Update config with new options")
        print("  her_agent config path      Show config file path")
        print("  her_agent config env-path  Show .env file path")
        sys.exit(1)
