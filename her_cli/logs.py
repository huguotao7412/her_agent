"""``her_agent logs`` — view and filter Hermes log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.her_agent/logs/``.

Usage examples::

    her_agent logs                    # last 50 lines of agent.log
    her_agent logs -f                 # follow agent.log in real time
    her_agent logs errors             # last 50 lines of errors.log
    her_agent logs gateway -n 100    # last 100 lines of gateway.log
    her_agent logs --level WARNING    # only WARNING+ lines
    her_agent logs --session abc123   # filter by session ID substring
    her_agent logs --component tools  # only tool-related lines
    her_agent logs --since 1h         # lines from the last hour
    her_agent logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hermes runs — try 'her_agent chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'her_agent chat' to generate logs)")
