"""``her_agent plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.her_agent/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from her_cli.config import get_env_value, save_env_value  # noqa: F811
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]her_agent plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `her_agent plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  her_agent gateway restart[/dim]")
    console.print()
...
def _plugin_exists(name: str) -> bool:
    ...
    import her_cli
    repo_plugins = _P(her_cli.__file__).resolve().parent.parent / "plugins"
...
def _discover_all_plugins() -> list:
    ...
    import her_cli
    repo_plugins = Path(her_cli.__file__).resolve().parent.parent / "plugins"
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] her_agent plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] her_agent plugins")
    console.print("[dim]Enable/disable:[/dim] her_agent plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch her_agent plugins subcommands."""
