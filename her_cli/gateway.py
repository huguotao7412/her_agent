"""
Gateway subcommand for her_agent CLI.

Handles: her_agent gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when her_constants is cached from a pre-update version during `her_agent update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "her_agent-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "her_cli.main gateway",
        "her_cli.main --profile",
        "her_cli.main -p",
        "her_cli/main.py gateway",
        "her_cli/main.py --profile",
        "her_cli/main.py -p",
        "her_agent gateway",
        "gateway/run.py",
    ]
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``her_agent update``
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `her_agent gateway`")
...
from her_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "her_agent-gateway"
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.her_agent/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Default ``~/.her_agent`` returns ``her_agent-gateway`` (backward compatible).
    Profile ``~/.her_agent/profiles/coder`` returns ``her_agent-gateway-coder``.
...
# her_agent-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (her_agent-gateway-*.service) and unrelated third-party
# "her_agent" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("her_agent.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``her_agent.service`` before the rename to
    ``her_agent-gateway.service``). When both a legacy unit and the current
    ``her_agent-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``her_agent-gateway-coder.service`` and unrelated third-party
      ``her_agent-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``her_agent.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current her_agent-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    her_agent gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: her_agent gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo her_agent gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    her_agent gateway uninstall")
    print_info("    sudo her_agent gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo her_agent gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo her_agent gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo her_agent gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.her_agent`` → ``ai.her_agent.gateway.plist`` (backward compatible).
    Profile ``~/.her_agent/profiles/coder`` → ``ai.her_agent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.her_agent.gateway-{suffix}" if suffix else "ai.her_agent.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.her_agent/her_agent-agent  -> /home/alice/.her_agent/her_agent-agent
      /opt/her_agent                 -> /opt/her_agent  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.her_agent                    → /home/alice/.her_agent
      /root/.her_agent/profiles/coder     → /home/alice/.her_agent/profiles/coder
      /opt/custom-her_agent               → /opt/custom-her_agent  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".her_agent").resolve()
    target_default = Path(target_home_dir) / ".her_agent"

    # Default ~/.her_agent → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.her_agent → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.her_agent) — keep as-is
...
User={username}
Group={group_name}
ExecStart={python_path} -m her_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (her_agent.service from pre-rename installs)
    # before installing the new her_agent-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}her_agent gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}her_agent gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}her_agent gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}her_agent gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}her_agent gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}her_agent gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.her_agent.gateway-{suffix}" if suffix else "ai.her_agent.gateway"
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  her_agent gateway status             # Check status")
    from her_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `her_agent gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: her_agent gateway start")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @her_agent:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. her_agent) and copy the bot token",
    ...
    },
    {
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Hermes will use (e.g., her_agent@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: her_agent pairing generate bluebubbles",
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'her_agent pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: her_agent pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'her_agent gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from her_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from her_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.her_agent/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `her_agent gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `her_agent pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `her_agent pairing approve`.")
    elif access_idx == 1:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: her_agent gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: her_agent gateway install")
                else:
                    print_info("  You can install later: her_agent gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo her_agent gateway install --system")
                    print_info("  Or run in foreground:  her_agent gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: her_agent gateway run")
                print_info("  For persistence:   tmux new -s her_agent 'her_agent gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: her_agent gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup her_agent gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: her_agent gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'her_agent gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: her_agent gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: her_agent gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s her_agent 'her_agent gateway run'")
                print()
            ...
            print()
            print("  her_agent gateway run                              # direct foreground")
            print("  tmux new -s her_agent 'her_agent gateway run'         # persistent via tmux")
            print("  nohup her_agent gateway run > ~/.her_agent/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: her_agent gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: her_agent gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: her_agent gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: her_agent gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    her_agent gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: her_agent gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  her_agent gateway install")
                    print("  sudo her_agent gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  her_agent gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup her_agent gateway run > ~/.her_agent/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s her_agent 'her_agent gateway run'         # persistent via tmux")
                    print("  nohup her_agent gateway run > ~/.her_agent/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  her_agent gateway install  # Install as user service")
                    print("  sudo her_agent gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Hermes gateway unit files from
        # pre-rename installs (e.g. her_agent.service). Profile units and
        # unrelated third-party services are never touched.
