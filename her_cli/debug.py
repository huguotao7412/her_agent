"""``her_agent debug`` — debug tools for Hermes Agent.

Currently supports:
    her_agent debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.her_agent/pastes/pending.json``.
    ...
    ran ``her_agent debug share`` repeatedly.  We now persist the schedule
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``her_agent debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``her_agent debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `her_agent debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "her_agent-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``her_agent debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.her_agent/pastes/pending.json``
    and rely on opportunistic sweeps (``_sweep_expired_pastes``) called from
    every ``her_agent debug`` invocation.  If the user never runs ``her_agent debug``
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"her_agent debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "her_agent-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _resolve_log_path(log_name: str) -> Optional[Path]:
    ...
    """
    from her_cli.logs import LOG_FILES
...
def _read_log_tail(log_name: str, num_lines: int) -> str:
    ...
    from her_cli.logs import _read_last_n_lines
...
def _capture_dump() -> str:
    """Run ``her_agent dump`` and return its stdout as a string."""
...
def collect_debug_report(*, log_lines: int = 200, dump_text: str = "") -> str:
    ...
    dump_text
        Pre-captured dump output.  If empty, ``her_agent dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  her_agent debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: her_agent debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'her_agent debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``her_agent debug`` call.
    ...
    # best-effort — any failure is swallowed so ``her_agent debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: her_agent debug <command>")
        print()
        print("Commands:")
