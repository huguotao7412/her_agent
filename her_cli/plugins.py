...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with her_agent-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.her_agent/plugins/<name>/``
3. **Project plugins** – ``./.her_agent/plugins/<name>/`` (opt-in via
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``her_agent honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``her_agent <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.her_agent/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # Repo-shipped generic plugins live next to her_cli/.  Memory and
        ...
        # show up in `her_agent plugins`) but only loaded when added to
        ...
        # 2. User plugins (~/.her_agent/plugins/)
        user_dir = get_hermes_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.her_agent/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".her_agent" / "plugins"
        ...
        for manifest in winners.values():
            ...
            if enabled is None or manifest.name not in enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = "not enabled in config (run `her_agent plugins enable {}` to activate)".format(
                    manifest.name
                )
                self._plugins[manifest.name] = loaded
                logger.debug(
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``her_agent tools`` TUI so plugin-provided toolsets appear
