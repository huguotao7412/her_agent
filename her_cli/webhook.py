"""her_agent webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    her_agent webhook subscribe <name> [options]
    her_agent webhook list
    her_agent webhook remove <name>
    her_agent webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.her_agent/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: her_agent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'her_agent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: her_agent webhook {subscribe|list|remove|test}")
        print("Run 'her_agent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (her_agent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: her_agent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from her_agent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (her_agent gateway run)")
