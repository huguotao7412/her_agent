...
Usage:
    python -m her_cli.main web          # Start on http://127.0.0.1:9119
    python -m her_cli.main web --port 8080
"""
...
# the canonical ``her_agent auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.her_agent/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "her_agent auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "her_agent auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "her_agent auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "her_agent auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from her_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from her_cli.auth import clear_provider_auth
...
#          → persists to ~/.her_agent/.anthropic_oauth.json AND credential pool
...
# Guarded so her_agent web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``her_agent auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "her_agent-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from her_cli import auth as hauth
    if provider_id == "nous":
        from her_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from her_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from her_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from her_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from her_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from her_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from her_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
def _discover_user_themes() -> list:
    """Scan ~/.her_agent/dashboard-themes/*.yaml for user-created themes."""
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as her_cli.plugins):
    1. User plugins:    ~/.her_agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.her_agent/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if os.environ.get("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".her_agent" / "plugins", "project"))
