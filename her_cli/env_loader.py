...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``her_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from her_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.her_agent/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("HERMES_HOME", Path.home() / ".her_agent"))
