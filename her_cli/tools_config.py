...
`her_agent tools` and `her_agent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.her_agent/cli-config.yaml under
...
from her_cli.nous_subscription import (
...
from her_cli.cli_output import (  # noqa: E402 — late import block
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from her_cli.plugins import discover_plugins, get_plugin_toolsets
...
from her_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/her_agent-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in her_agent-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "her_agent-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "her_agent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `her_agent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by her_agent tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (her_agent-cli, her_agent-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `her_agent tools` and `her_agent setup tools`.
    ...
    print(color("  Changes take effect on next 'her_agent' or gateway restart.", Colors.DIM))
    print()
