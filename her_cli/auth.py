...
is persisted in ~/.her_agent/auth.json with cross-process file locking.
...
from her_cli.config import get_hermes_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_CLIENT_ID = "her_agent-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from her_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `her_agent model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.her_agent/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `her_agent logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'her_agent doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'her_agent model' for available providers, or run 'her_agent doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'her_agent model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.her_agent/.env.",
...
#
# Tokens live in ~/.her_agent/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `her_agent auth list` / `her_agent status`."""
    try:
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.her_agent/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.her_agent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `her_agent auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `her_agent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `her_agent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `her_agent auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.her_agent/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `her_agent auth` to re-authenticate."
            )
            relogin_required = True
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "her_agent-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``her_agent auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``her_agent auth add nous --label <name>``).  It gets embedded in the
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `her_agent status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``her_agent auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `her_agent auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `her_agent auth` and
    # `her_agent model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def login_command(args) -> None:
    """Deprecated: use 'her_agent model' or 'her_agent setup' instead."""
    print("The 'her_agent login' command has been removed.")
    print("Use 'her_agent auth' to manage credentials,")
    print("'her_agent model' to select a provider, or 'her_agent setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.her_agent/auth.json."""
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `her_agent model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `her_agent model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `her_agent model` or configure an API key to use Hermes.")
    else:
