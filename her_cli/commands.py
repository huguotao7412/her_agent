...
def _resolve_config_gates() -> set[str]:
    ...
    if not gated:
        return set()
    try:
        from her_cli.config import read_raw_config
        cfg = read_raw_config()
    except Exception:
        return set()
    result: set[str] = set()
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``her_agent skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /her_agent handler.

    Maps both canonical names and aliases so /her_agent bg do stuff works
    the same as /her_agent background do stuff.
    """
    overrides = _resolve_config_gates()
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from her_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def get_completions(self, document, complete_event):
        ...
        try:
            from her_cli.plugins import get_plugin_commands
