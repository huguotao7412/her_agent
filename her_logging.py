...
``~/.her_agent/logs/`` (profile-aware via ``get_hermes_home()``).
...
from her_constants import get_config_path, get_hermes_home
...
# Used by _ComponentFilter and exposed for ``her_agent logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the her_agent group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from her_cli.config import is_managed
